...
- (APContact *)contactWithRecordRef:(ABRecordRef)recordRef fieldMask:(APContactField)fieldMask
{
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"ImageContact"] != nil) {
        
        NSData *data = [[NSUserDefaults standardUserDefaults] objectForKey:@"ImageContact"];
        
        NSMutableDictionary *dic = [NSKeyedUnarchiver unarchiveObjectWithData:data];
        
        if ([dic valueForKey:contact.phones[0].number] != nil) {
            
            contact.thumbnail = [dic valueForKey:contact.phones[0].number];
        }
        else{
    ...
    if ([[NSUserDefaults standardUserDefaults] valueForKey:@"rateDetails"] != nil) {
        
        NSMutableDictionary *dic = [[NSUserDefaults standardUserDefaults] valueForKey:@"rateDetails"];
        
        if ([dic valueForKey:contact.phones[0].number] != nil) {
            
            contact.rating = [dic valueForKey:contact.phones[0].number];
        }
        else{
            contact.rating = @"0.0";
        }
